...
def reconcile(request: ReconcileRequest) -> ReconcileReport:
    known_members = {m.id for m in request.group.members}
    by_id = {}
    duplicate_ids = set()
    exceptions = []
    ...
    for event in request.events:
        ...
        if target is None:
            ...
            invalid_corrections.add(event.id)
            continue
    ...
    for event in request.events:
        if event.id in duplicate_ids or event.id in superseded or event.id in invalid_corrections:
            continue
        if event.confidence < request.confidence_threshold:
            ...
            continue
        if event.member_id is not None and event.member_id not in known_members:
            exceptions.append(make_exception(event, "unknown_member", f"Member '{event.member_id}' is not in this group."))
            continue
        if event.event_type not in FINANCIAL_TYPES:
            continue
        ...
        key = (event.member_id, event.event_type.value, event.amount_minor, event.currency, event.source_text.strip().lower())
        if key in seen_keys:
        ...
        totals[event.event_type.value] += event.amount_minor
